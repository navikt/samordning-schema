...
bump-version:
	...
	$(DOCKER) run --rm -t \
		...
		-v ${HOME}/.m2:/var/maven/.m2 \
		-e MAVEN_CONFIG=/var/maven/.m2 \
		maven:3.6-jdk-12 mvn -Duser.home=/var/maven versions:set -DnewVersion=$$(cat ./VERSION) -DgenerateBackupPoms=false -B -e
...
maven-deploy:
	$(DOCKER) run --rm -t \
		...
		-v ${HOME}/.m2:/var/maven/.m2 \
		-e MAVEN_CONFIG=/var/maven/.m2 \
		maven:3.6-jdk-12 mvn -Duser.home=/var/maven deploy -B -e
